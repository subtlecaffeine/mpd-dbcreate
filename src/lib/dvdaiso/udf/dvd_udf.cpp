                         ...
                         struct Partition *partition, struct AD *ad )
{
  ...
  while( p < 176 + L_EA + L_AD ) {
    ...
    switch( flags & 0x0007 ) {
    case 0:
      UDFShortAD( &data[ p ], &ad_i, partition );
      p += 8;
      break;
    case 1:
      UDFLongAD( &data[ p ], &ad_i );
      p += 16;
      break;
    case 2:
      UDFExtAD( &data[ p ], &ad_i );
      p += 20;
      break;
    case 3:
      switch( L_AD ) {
...
static int UDFGetPVD(dvd_reader_t *device, struct pvd_t *pvd)
{
  ...
  if(!UDFGetDescriptor( device, 1, pvd_buf, &pvd_buf_base[DVD_VIDEO_LB_LEN + 2048] - pvd_buf))
    return 0;
